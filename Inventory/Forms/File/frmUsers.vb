...
Public Class frmUsers
    ...
    Private Function AllValidFields() As Boolean
        ...
        If txtUserID.Text.ToString <> "" And txtUserName.Text.ToString <> "" And _
           txtPassword.Text.ToString <> "" And txtIRoleID.Text.ToString <> "" Then
            retval = True
        End If
        Return retval
    End Function
    Private Sub Sub_Insert()
        Try
            ...
            Dim USERID As New SqlParameter("@USERID", SqlDbType.VarChar, 10) : USERID.Direction = ParameterDirection.Input : USERID.Value = txtUserID.Text : Params(0) = USERID
            Dim USERNAME As New SqlParameter("@USERNAME", SqlDbType.VarChar, 50) : USERNAME.Direction = ParameterDirection.Input : USERNAME.Value = txtUserName.Text : Params(1) = USERNAME
            Dim PASSWORD As New SqlParameter("@PASSWORD", SqlDbType.VarChar, 15) : PASSWORD.Direction = ParameterDirection.Input : PASSWORD.Value = txtPassword.Text : Params(2) = PASSWORD
            Dim IROLEID As New SqlParameter("@IROLEID", SqlDbType.Int, 10) : IROLEID.Direction = ParameterDirection.Input : IROLEID.Value = Convert.ToInt16(txtIRoleID.Text) : Params(3) = IROLEID
            ...
            If ItemExists() Then
                MsgBox("User Id : " & txtUserID.Text & ", User Name : " & txtUserName.Text & " already exists!")
            Else
    ...
    Private Sub DataGrid1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles DataGrid1.Click, DataGrid1.CurrentCellChanged
        Try
            With DataGrid1
                ...
                txtUserID.Text = CStr(.Item(.CurrentCell.RowNumber, 1))
                txtUserName.Text = CStr(.Item(.CurrentCell.RowNumber, 2))
                txtPassword.Text = CStr(.Item(.CurrentCell.RowNumber, 3))
                txtIRoleID.Text = CStr(.Item(.CurrentCell.RowNumber, 4))
                txtRole.Text = CStr(.Item(.CurrentCell.RowNumber, 5))
    ...
    Private Function ItemExists() As Boolean
        ...
        CODE.Value = txtUserID.Text.ToString.Trim
        Params(0) = CODE

        Dim NAME As New SqlParameter("@NAME", SqlDbType.VarChar, 50)
        NAME.Direction = ParameterDirection.Input
        NAME.Value = txtUserName.Text.ToString.Trim
        Params(1) = NAME
    ...
    Private Sub btnRoleLookUp_Click(sender As Object, e As EventArgs) Handles btnRoleLookUp.Click
        gCode = txtIRoleID.Text : gDesc = txtRole.Text
